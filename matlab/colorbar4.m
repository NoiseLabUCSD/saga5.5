...
if loc(1)=='v', % Append vertical scale to right of current plot
    ...
    if isempty(ax),
        ...
        pos = get(h,'Position'); 
        [az,el] = view;
        stripe = 0.075/1.5; edge = 0.02; 
        if all([az,el]==[0 90]), space = 0.05; else space = .1/2; end
        set(h,'Position',[pos(1) pos(2) pos(3)*(1-stripe-edge-space) pos(4)])
        rect = [pos(1)+(1-stripe-1.5*edge)*pos(3) pos(2) stripe*pos(3)/2 0.5*pos(4)];
        ud.origPos = pos;
...
elseif loc(1)=='h', % Append horizontal scale to top of current plot
    ...
    if isempty(ax),
        units = get(h,'units'); set(h,'units','normalized')
        pos = get(h,'Position')
        stripe = 0.075/1.5; space = 0.1/2;
        set(h,'Position',...
            [pos(1) pos(2)+(stripe+space)*pos(4) pos(3) (1-stripe-space)*pos(4)])
        rect = [pos(1)+pos(3)*0.75 pos(2) pos(3)/4 stripe*pos(4)/2];
        ud.origPos = pos;
