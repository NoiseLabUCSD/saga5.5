clear all
mm = 8000; % mm: # of samples in one fort.81
filen = 'f195hla_oS1_gm';
...
for ij = 1: nfile 
   unix(['cp fort.81_' num2str(ij) ' fort.81']);
   ...
   if iplot & ij == nfile
      ...
      H = colorbar('horiz','XAxisLocation','top');
      pos = get(H,'position');
      set(H,'position',...
            [pos(1)+pos(3)/3*2 pos(2)+pos_fig(4)*1.09 pos(3)/3 pos(4)*0.65])
      text(pos(1)+pos(3)/3*2, pos(2)+pos_fig(4)*1.35,'pdf',...
