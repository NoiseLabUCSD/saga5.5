...
C     IO-BUFFERS. RETURNS # BLOCKS COMPATIBLE WITH LAST PARAMETER
C     IN CALL TO OPNBUF.
       MEMLFT=IFIX( (0.5*(MSIZ-ISOLD+1)-63)/64)
       RETURN
       END
...
C**************************************************************
C
C     TIMING ROUTINES DEPENDENT ON THE OPERATING SYSTEM
C
      SUBROUTINE CLTIME
C      DIMENSION SECS(2)
C      COMMON /TIMECT/ OLDSECS,FSECS,IDATE
C      IDATE=0      
C      CALL GETTIM(IH,IM,IS,IHU)
C      OLDSECS=(IH*60.0+IM)*60.0+IS+0.01*IHU
C      FSECS=OLDSECS
CUNIX OLDSECS = ETIME(SECS)
CVMS   OLDSECS=SECS(1)
...
C      COMMON /TIMECT/ OLDSECS,FSECS,IDATE
C      CALL GETTIM(IH,IM,IS,IHU)
C      SECS(1)=(IH*60.0+IM)*60.0+IS+0.01*IHU
C      IF (SECS(1).LT.FSECS) IDATE=IDATE+1
C      FSECS=SECS(1)
C      DT=SECS(1)-OLDSECS+IDATE*3600.0*24.0
      INTEGER*4 IT4
      REAL*8 IT8
      EQUIVALENCE (IT4,IT8)
      CALLSTAT=LIB$STAT_TIMER(2,IT4)
      T1=1E-2*IT4
      RETURN
      END
c**********************************************************************
...
C***  ultrix 
c      WRITE(ENVVAR,'(A3,I3.3)') 'FOR',IUN
c      CALL GETENV(ENVVAR,FILENM)
c      IF (FILENM.EQ.' ') THEN
...
c     1     ERR=300)
      RETURN
 300  IOER=1
       write(*,*) 'unit=',iun,' Could not be opened'
       stop
c      RETURN
C
...
c      CALL GETENV(ENVVAR,FILENM)
c      IF (FILENM.EQ.' ') THEN
c       OPEN(UNIT=IUN,STATUS='OLD',FORM='FORMATTED',ERR=400)
c      ELSE
c       OPEN(UNIT=IUN,FILE=FILENM,STATUS='OLD',
c     &      FORM='FORMATTED',ERR=400)
c      END IF
C***  MS-DOS         
c      IF (IUN.LT.10) THEN
c        WRITE(FILENM,100) IUN
...
C
C***  VMS
      OPEN(UNIT=IUN,STATUS='NEW',FORM='UNFORMATTED',ERR=300)
C***  ultrix 
c      WRITE(ENVVAR,'(A3,I3.3)') 'FOR',IUN
c      CALL GETENV(ENVVAR,FILENM)
c      IF (FILENM.EQ.' ') THEN
c       OPEN(UNIT=IUN,STATUS='UNKNOWN',FORM='UNFORMATTED',ERR=300)
c      ELSE
c       OPEN(UNIT=IUN,FILE=FILENM,STATUS='UNKNOWN',
