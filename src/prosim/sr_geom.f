...
c  
      ...
      integer*4 nsrcx,jrec,jrng,j,jj,jsr,jzs,
     .   iibad,nzsr0,iisort,isum,idx,jdx,displcm,dplcm
      integer*4 jk,ik,jndx(NVRMAX),z_indx(NVRMAX)
      real*4 zsrx(NVRMAX),zdpth(NVRMAX)
      real*4 c0,k0,twpie
      real*4 dz1,dz2,hs
      real*4 beta1,beta2
      real*8 zapp(NVRMAX),rlast
      complex*8 ibeta1,ibeta2
      complex*16 cfac
c
...
c: mzsrc(jzs) (jzs=1:nzs) gives the index of zsrc(jzs) in zsr:
         jsr=1
         do while (zsrx(jsr+dplcm) .ne. zsrc(jzs))
           jsr=jsr+1
         enddo
cc fbv         call hunt(zsrx,nzsr,dble(zsrc(jzs)),jsr)
cc fbv         if(zsrx(jsr) .ne. zsrc(jzs)) jsr=jsr + 1
         mzsrc(jzs+displcm)=jsr+dplcm
       enddo
cc fbv(!)      endif
       do jrec=1,nrecusr
         jsr=1
c: mzrec(jrec) (jrec=1:nrec) gives the index of zrec(jrec) in zsr:
         do while (zsrx(jsr+dplcm) .ne. zrec(jrec+displcm))
           jsr=jsr+1
         enddo
cc fbv         call hunt(zsrx,nzsr,dble(zrec(jrec+displcm)),jsr)
cc fbv         if(zsrx(jsr) .ne. zrec(jrec+displcm)) jsr=jsr + 1
c fbv         mzrec(jrec+displcm)=jsr+dplcm
         mzrec(jrec+displcm)=jsr+dplcm
       enddo
ccc
       if(nduct .gt. 1) then
         do j=1,nduct
...
c
       ...
       enddo
       call hpsort_indx(nzsr0,zapp,z_indx)

       do jrec=1,nzsr0
c         zsr(jrec+dplcm)=zsrx(jrec)
c fbv         zsr(jrec+dplcm)=zsrx(jrec+dplcm)
...
c
       do j=1,nrng
         sq2pir(j)=cfac/dsqrt(range(j))
       enddo
      endif
c
cpln      rmin=rkm(1)/1000.
c
