...
c
     ...
     .   phztop,phzbot,gamphi,phz_est,phi_re,dphi_re,pie
      data rzero/0.d0/
c
...
c: cumulative downward propagator matrix (see p. 44, ORCA II):
     ...
     .      h_exp(1,j),
     .      rho_prod(2,j),rhofac(j),Dphi_w(2,j),Ddphi_w(2,j),
     .      Dphi_w(1,jp1),Ddphi_w(1,jp1),Dphi_sw,Ddphi_sw,vg)
      enddo
c
...
c: cumulative upward propagator matrix (see p. 44, ORCA II):
     ...
     .      g_exp(1,j),
     .      rho_prod(1,j),rhofac(j),Dphi_w(2,j),Ddphi_w(2,j),
     .      Dphi_w(1,jp1),Ddphi_w(1,jp1),Dphi_sw,Ddphi_sw,vg)
      enddo
