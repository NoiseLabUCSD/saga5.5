...
c:**********************************************
      subroutine rx_ek_calc(k,e_k,phm,ndv,iifail)
c
...
c
      ...
      include 'i_o_opt_com'
      include 'i_o_1b_com'
      common /out_com3/ nzsr,nlay,dmmy,dmmy3
      integer*4 nzsr,nlay,dmmy,dmmy3,iifail
      include 'gen1_com'
c: Local variables:
      integer*4 ndv,ndva,j,jm1,jdv
      real*8 k,e_k(6),phm,dzero,g12_k,g22_k,g12_w,g22_w,
     .   g12_kk,g22_kk,capx_k,capx_w,capx_fac,gamsq,
     .   g12x,g22x,pigr
c
c: TEMP
      data dzero/0.d0/
...
cpln      write(6,*)
      do j=nlay-1,2,-1
         jm1=j-1
cpln         write(6,*)'Layer #',j
cpln         write(6,*)'Layer thickness: ',h(j)
         call rx_ep_calc(xkh,w,wsq,xkhsq,xksq(1,j),xksq(2,j),eta(j),
     .      etasq(j),h(j),u11(1,j),u12(1,j),u21(1,j),u22(1,j),
     .      u_exp(1,j),isp(j),ndv,ndva,ailay(1,1,1,j),bilay(1,1,1,j),
     .      xilay(1,j),zetalay(1,1,j),1,2,phtot)
c     .      u_exp(1,j),isp(j),ndv,ndva,ailay(1,j),bilay(1,j),
c     .      u_exp(1,j),isp(j),ndv,ndva,ailay(1,1,1,j),bilay(1,1,1,j),
55       continue
     ...
     .      g11(1,1,j),g12(1,1,j),g21(1,1,j),g22(1,1,j),ndv)
         ...
         enddo
c: Take G across interface from layer j to layer jm1:
         call rx_iface(g11(1,1,j),g12(1,1,j),g21(1,1,j),g22(1,1,j),
     .      g11(1,2,jm1),g12(1,2,jm1),g21(1,2,jm1),g22(1,2,jm1),
     .      rhofac(jm1),ndv)
      enddo
c
C hp      gamsq=dreal(xksq(1,nlay)) - dreal(xkhsq)
c      gamsq=dreal(xksq(1,nlay)) - real(xkhsq)
      gamsq=xksq(1,nlay) - xkhsq
      xilay(1,nlay)=gamsq
      xilay(2,nlay)=gamsq
c Sometimes problems that gamsq positive
      if(gamsq .ge. 0.0) then
        iifail = 1
        return
      end if
      migam2(1)=dsqrt(-gamsq)
      g12x=g12(1,2,1)
      g22x=g22(1,2,1)
...
c: (see ORCA II, p. 43).
      ...
      include 'gen1_com'
      include 'gen2_com'
      integer*4 jlay,ndv,j,jp1,jdv,jlaymax,j1,j2,kd,ntry,
     .   iipol,iidone,ii
      real*8 rho_ref,dzero,phmx,err(25),errmin,vg,Dvg_w,D2k_Df,
     .   knr,magsq,Lmag,twpie
      complex*16 igam0(6),igam1(6),igam2(6),N_n,DN_n,DR1_w,
     .   Aigam(6),DWrk_w,Digam1,Digam2,Digam0,DTC2,Aifac,vgc
      data dzero/0.d0/, twpie/6.28318530717959d0/
c
...
10    continue
      ntry=0
      call rx_igam_calc(xksq(1,nlay),xkh,xkhsq,w,wsq,igam2)
      call rx_igam_calc(xksq(1,2),xkh,xkhsq,w,wsq,igam0)
c
      jlay=nsvmin
      ii=isvmin
      jlaymax=jlay
...
c: Vertical wavenumbers at match point:
      call rx_igam_calc(xksq(ii,jlay),xkh,xkhsq,w,wsq,igam1)
c: Compute downward-looking reflection coefficient:
      call rx_rp_flay(g11(1,ii,jlay),g12(1,ii,jlay),g21(1,ii,jlay),
     .   g22(1,ii,jlay),igam1,igam2,rho_prod(1,jlay),ndv,
     .   R1,TC1)
c: Compute upward-looking reflection coefficient:
      call rx_rp_vac(h11(1,ii,jlay),h12(1,ii,jlay),igam1,igam0,
     .   rho_prod(2,jlay),ndv,R2,
     .   TC2,-1.d0,jlay,ii)
c
...
c: Polish root to see if more accurate value of kn needed:
            ...
            call rx_ek_calc(kn(nmode),ekn(1,nmode),phmx,ndv,iifail)
            if(iifail .gt. 0) return
            mode_phz(1,nmode)=phmx
            ncalc(nmode)=ncalc(nmode) + 1
            iipol=1
            if(iidiag .gt. 0) print *,'Polished roots ...',
     .   (err(j),j=1,nduct)
            goto 10
...
c: Polish root again to see if more accurate value of kn needed:
            ...
            ncalc(nmode)=ncalc(nmode) + 1
            iipol=2
            if(iidiag .gt. 0) print *,'Polished roots again ...',
     .   (err(j),j=1,nduct)
            goto 10
         else
...
c: Set flag so that this is the last try:
            iidone=1
      if(iidiag .gt. 0) print *,'Using best match point: ',nmode,jlay,
     .   (err(j),j=1,nduct)
c: FIX (4-1-96):
            goto 30
cc          goto 10
...
c: Total derivative of group velocity:
         Dvg_w=dreal((vgc*vgc*rx_r1r2(6) + 2.d0*vgc*rx_r1r2(4) +
     .      rx_r1r2(5))/rx_r1r2(2))
c: Second total derivative of kn w.r.t. f (see ORCA II, p.63):
         D2k_Df=(-(twpie/vgc)**2)*Dvg_w
...
c cmplx         Dphi_w(1,2)=dcmplx(0.d0,0.d0)
         Dphi_w(1,2)=0.d0
         DTC2=TC2(3) + TC2(2)/vgc
         Digam0=igam0(3) + igam0(2)/vgc
c: Correction by Westwood (DR1_w before Ddphi_w(1,2)) 16/10-97
         DR1_w=R1(3) + R1(2)/vgc
...
c cmplx         Dphi_w(1,nlay)=(TC1(3) + TC1(2)/vgc)*N_n + TC1(1)*DN_n
         Dphi_w(1,nlay)=dble((TC1(3) + TC1(2)/vgc)*N_n + TC1(1)*DN_n)
         Digam2=igam2(3)+igam2(2)/vgc
c cmplx         Ddphi_w(1,nlay)=Digam2*philay(1,nlay) +
c cmplx     .      igam2(1)*Dphi_w(1,nlay)
         Ddphi_w(1,nlay)=dble(Digam2*philay(1,nlay) +
     .      igam2(1)*Dphi_w(1,nlay))
c: Compute total derivative w.r.t. w of phi,dphi at reference depth:
cc       philay(ii,jlay)=N_n*(1.d0 + R1(1))
c cmplx         Dphi_w(ii,jlay)=N_n*DR1_w + DN_n*(1.d0 + R1(1))
         Dphi_w(ii,jlay)=dble(N_n*DR1_w + DN_n*(1.d0 + R1(1)))
         Digam1=igam1(3) + igam1(2)/vgc
c cmplx         Ddphi_w(ii,jlay)=-igam1(1)*N_n*DR1_w + 
c cmplx     .      (igam1(1)*DN_n + Digam1*N_n)*(1.d0 - R1(1))
         Ddphi_w(ii,jlay)=dble(-igam1(1)*N_n*DR1_w + 
...
c      write(77,*)eig_char(4,nmode),eig_char(5,nmode)
      nzref(nmode)=kd
      eig_char(1,nmode)=vgc
      eig_char(2,nmode)=Dvg_w
c      write(6,*)nmode,eig_char(1,nmode)
c      write(6,*)nmode,eig_char(2,nmode)
c      write(6,*)nmode,eig_char(3,nmode)
c      write(6,*)nmode,eig_char(4,nmode)
c      write(6,*)nmode,eig_char(5,nmode)
c      pause
...
ccc
      subroutine rx_igam_calc(Ksq,xkh,xkhsq,w,wsq,igam0)
c
      implicit none
...
ccc
     ...
     .   e11,e12,e21,e22,zzexp,iso,ndv,ndva,ailay,bilay,xilay,zetalay,
     .   ii1,ii2,phtot)
c
...
c: For ndv=-5, the k and kk derivatives only are computed.
...
c 
      implicit none
      integer*4 iso,ndv,ndva,ii1,ii2
      real*8 xkh,xkhsq,xk1sq,xk2sq,eta,etasq,e11(6),e12(6),e21(6),
     .   e22(6),zzexp(6),xilay(2),phlay,phtot,pigr,twpie
     ...
     .   khprodsq,Kfac,Kfac_w,Kfac_k,dh_dw,d2h_dw
      complex*16 zetalay(2)
      real*8 ailay(2,2),bilay(2,2)
      data dzero/0.d0/,pigr/3.14159265358979d0/
      data twpie/6.28318530717959d0/
c
      if(iso .eq. 1 .or. iso .eq. -1) then
c: Compute propagator matrix for isospeed layer:
cpln         write(6,*)'Compute propagator matrix for isospeed layer'
         gamsq=xk1sq - xkhsq
         xilay(1)=gamsq
         xilay(2)=gamsq
         khprod=xkh*h
         if(abs(gamsq) .lt. 1.d-11) then
cpln         write(6,*)'Plane wave propagating almost horizontal'
...
cpln            write(6,*)'Plane wave propagating'
            gam=dsqrt(gamsq)
            gamh=h*gam
            cosgamh=dcos(gamh)
            singamh=dsqrt(1.d0 - cosgamh*cosgamh)
            if(mod(gamh,twpie) .gt. pigr) singamh=-singamh
            e11(1)=cosgamh
            e22(1)=cosgamh
            e12(1)=singamh/gam
            e21(1)=-gam*singamh
...
cpln            write(6,*)'Plane wave is evanescent'
            igam=dsqrt(-gamsq)
            igamh=igam*h
            ei2gamh=0.5d0*dexp(-2.d0*igamh)
...
c fbv            zetalay(ii1)=dcmplx(0.d0,phlay)  !done
c fbv            zetalay(ii2)=dcmplx(0.d0,phlay)  !done
         endif
      else
c: Compute propagator matrix for Airy layer:
c: Compute Airy function arguments at top and bottom of layer:
cpln         write(6,*)'Compute propagator matrix for Airy layer'
         mgam1sq=xkhsq - xk1sq
         mgam2sq=xkhsq - xk2sq
         xi1=mgam1sq/etasq
         xi2=mgam2sq/etasq
c: Compute Airy functions of real arguments (ai and bi):
         ...
         if(xi1 .lt. 0.d0) then
            gam1cub=-dsqrt(-mgam1sq)*mgam1sq
         else
            gam1cub=0.d0
         endif
         if(xi2 .lt. 0.d0) then
            gam2cub=-dsqrt(-mgam2sq)*mgam2sq
         else
            gam2cub=0.d0
         endif
         phlay=abs(gam1cub - gam2cub)/(1.5d0*abs(eta)*etasq)
         phtot=phtot + phlay
c
         ailay(1,ii1)=ai1
         ailay(1,ii2)=ai2
...
cpln         write(6,*)
cpln         write(6,*)
cpln         write(6,*)'x1,x2: ',xi1,xi2
cpln         write(6,*)'zeta1,zeta2: ',zetalay(ii1),zetalay(ii2)
cpln         write(6,*)'a1,a2: ',ai1,ai2
cpln         write(6,*)'b1,b2: ',bi1,bi2

c
c: Compute propagator matrix for Airy layer:
         call rx_airy_prop(xi1,ai1,aip1,bi1,bip1,zzexp1,
     .      xi2,ai2,aip2,bi2,bip2,zzexp2,eta,e11,e12,e21,e22,
     .      zzexp,xkh,xkhsq,xk1sq,xk2sq,etasq,w,wsq,ndv,ndva)
c
cpln         write(6,*)e11(1)
cpln         write(6,*)e12(1)
cpln         write(6,*)e21(1)
...
c      data det/0.31830988618379/
c
      det=0.31830988618379d0
...
c
      ...
      if(D(1) .eq. (0.d0,0.d0)) then
         ...
         do j=2,ndv
            ...
            TC(j)=(1.d0,0.d0) 
         enddo
cc       if(iidiag .gt. 0) print *,'Info msg: D=0 in rx_rp_flay'
         print *,'Info msg: D=0 in rx_rp_flay'
         return
      endif
      V(1)=N(1)/D(1)
...
cpln      write(6,*)
cpln      write(6,*)'e11: ',e11(1)
cpln      write(6,*)'e12: ',e12(1)
cpln      write(6,*)'e21: ',e21(1)
cpln      write(6,*)'e22: ',e22(1)
cpln      write(6,*)'j1,k1: ',j1(1),k1(1)
cpln      write(6,*)'gami1,gami2: ',gami1(1),gami2(1)
cpln      write(6,*)'V,TC: ',V(1),TC(1)
c
...
ccc
      subroutine rx_rp_vac(e11,e12,gami1,gami2,
...
c
      ...
      real*8 e11(6),e12(6),rho_prod,sg
