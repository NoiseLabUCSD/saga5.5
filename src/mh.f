...
c     Peter Gerstoft 02
     ...
     &     total1(mpar),total2(mpar)
      ...
      EXTERNAL fork,wait
      INTEGER getpid,id
      INTEGER iloop,i,jp,ii,irot,lunfit,ierr,iparm
...
c     CALL opfilr(85,ioer)
            lunfit=85
            CALL mh(f1,ppd1,nye,cov1,total1,u,irot,bound,lunfit)
            CALL flush(lunfit)
            ID=getpid()
...
c     WRITE(*,*)'calling 2nd sampling', iloop
         ...
         xdum=ran2(iseed)       ! 
         lunfit=86
         CALL mh(f2,ppd2,nye,cov2,total2,u,irot,bound,lunfit)
         CLOSE(86)
c     
         retval=wait(status) 
...
C     WRITE out
      ...
      DO ii=1,nparm
         IF ((iopt(14).EQ.1) ) THEN
            IF (par2phy(ii) .EQ. 9 .and. iopt(30) .ne. 11) THEN ! source range
               WRITE(40,810)' f_min(',ii,')=',fmin(ii)/1000,';'
               WRITE(40,810)' f_max(',ii,')=',fmax(ii)/1000,';'
            ELSE
               WRITE(40,810)' f_min(',ii,')=',fmin(ii),     ';'
               WRITE(40,810)' f_max(',ii,')=',fmax(ii),     ';'
            ENDIF
            WRITE(40,'(a,i2,a,i3,a)')'par2phy(',ii,')=',
     &           par2phy(ii),';'
         ELSE
            WRITE(40,'(a,i2,a,i3,a)')'par2phy(',1 ,')=',
     &           par2phy(1 ),';'
 ...
 810        FORMAT(a,i2,a,e15.6,a)
      ...
      WRITE(40,'(a)')'x=['
      DO jp=1,nppd
         WRITE(40,'(100g14.6)') (fmin(i)+deltaf(i)*(jp-0.5),
     &        i=1,nparm)
      ENDDO
      WRITE(40,'(a)')'];'
      DO i=1,nparm
      ...
      CALL system('/bin/mv -f samp1.mat ${filename}.mh1') 
      CALL system('/bin/mv -f samp2.mat ${filename}.mh2') 
      CLOSE(90) 
...
c***********************************************************      
      SUBROUTINE mh(fopt,ppd,nye,covar,total,u,irot,bound,lunfit)
c***********************************************************      
      USE global
      INCLUDE 'comopt.h'
...
c     functions
      REAL mhwalk,ran2,boxwalk,boxwalksmall
...
c     fopt(jmov)=     mhwalk(fold(jmov),df0(jmov),
...
c---- move is accepted!
               if (iopt(6)==1)
     &              WRITE(lunfit+12,'(30f13.3)')(fp(ipar),
     &              ipar=1,nparm),e
               iaccept=1+iaccept
...
c     WRITE(*,'(a,10e14.4)')'end one mh',(fr(jJ),jj=1,Nparm)
...
c     find rotation 
c pg may08     CALL svdcmp(u,nparm,nparm,mpar,mpar,cross2,sinv1)  
      CALL svdcmp(u,nparm,nparm,mpar,mpar,sinv1,cross2)  
...
c***********************************************************
      SUBROUTINE mhconv(ppd1,ppd2,epsmax)
c***********************************************************      
      USE global
      INCLUDE 'comopt.h'
      REAL ppd1(mpar,mdig), ppd2(mpar,mdig),epsmax,dfchi
      REAL diff1,tot,eps(mpar), tot1,tot2,propconv,gammq
      INTEGER i, jp, imax
      epsmax=0
      IF (1 .eq. 2) THEN
         tot1 = 0
...
C         write(*,*) 'tot1,tot2',tot1,tot2
     ...
     2              /(ppd1(i,jp) + ppd2(i,jp))
         ...
         ENDDO
         IF (diff1 .EQ. 0) THEN
            STOP ' mh: gaconv all pdp is zero'
         ENDIF
...
c***********************************************************      
      REAL FUNCTION mhwalk(xmod,dx,xmin,xmax,tmp)
...
c***********************************************************      
      USE global
      INCLUDE 'comopt.h'
      REAL t1,xtt(Mpar),func,pri,prior
...
c---  1 PARAMETER
 ...
 320           FORMAT(' >>> Estimated enumerative cpu time:',
     &              F12.3,' secs.')
            ...
            WRITE(80)(xtt(iparm),iparm=1,nparm),(func/numh-pri)
            if (iopt(6)==1)    
