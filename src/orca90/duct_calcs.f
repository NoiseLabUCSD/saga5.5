...
c
      ...
      use i_o_com
      use gen_com
      integer nm_ok,ndup,iduct,jm0,nm_temp,jm1,jm2,inc
...
c: Change reference depth back:
            ...
            call zref_chng
c
            call mfun_comp(nzsr,nmode,jm0,phiz,dphiz,nduct,
     .         mzduct,pct_diff)
...
c: Inserts mode into correct place and moves other up:
      implicit none
      integer*4 jfrom,jto,nmode,nzref(nmode),ncalc(nmode),iishn(nmode),
     .   nzsr,ji,jzsr
...
ccccccccccccccccccccccc   MRF   ccccccccccccccccccccccccccccccccccc
...
cMRF       write(6,*) 'jsave= ',jsave
cMRF       write(6,*) 'kn_indx(jsave)= ',kn_indx(jsave)
               jsave=jsave+1
            enddo
            call eig_insert(jsave,nmode+1,eig_char,nzref,ncalc,
     .         mode_phz,phi,dphi,psi,dpsi,exp_gbs,iishn,nm1,nzsr)
...
ccc
      subroutine mfun_comp(nzsr,nmode,jm0,phiz,dphiz,nduct,
     .   mzduct,pct_diff)
c
...
c
      ...
      complex*8 phiz(nzsr,nmode),dphiz(nzsr,nmode)
c
      psum1=0.0
      psum2=0.0
      dpsum1=0.0
...
cxx   pct_diff=min(psum1,psum2)/max(1.e-200,psum)
      pct_diff=max(min(psum1,psum2)/max(1.e-35,psum),
