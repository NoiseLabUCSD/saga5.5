      subroutine fix_path(k0,rr0,iileft,iilow)
c
      use parms_com
      use i_o_com
      use gen_com
      integer*4 iileft,iilow,j
...
c: Move along phase contour to |R1R2|=1 contour:
     ...
     .      dlnlast,k0,rr0,.05d0,0.19635d0)
         if(iifail .eq. 1) then
cMRF            print *,'Failure in traj_phase from fix_path: ',nmode,k0,rr0
            call stop_run
            return
         endif
     ...
     .      k0,(rr0(j,4),j=1,2)
         ...
         if(branch .ne. (0.d0,0.d0)) then
            ...
            if(dreal(delk) .lt. 0.d0 .or. dimag(delk) .lt. 0.d0) then
               k0=kcut0
               call sheet_init(k0,1,iish,iish_ref)
co      print *,'Calling cut_cross2 after bad traj_phase: ',k0/kw0
               call r1r2_calc(k0,rr0,2,0)
               call cut_cross2(k0,rr0)
            endif
         endif
      endif
      if((iileft .eq. 1 .and. dimag(rr0(2,4)) .ge. 0.d0) .or.
     .   iilow .eq. 1) then
...
cc       print *,'cut_cross: ',k0,(rr0(j,4),j=1,2)
         if(iidone .eq. 1) then
            nmode=nmode - 1
co            print *,'no crossing found '
            return
         elseif(iifail .eq. 1) then
            nmode=nmode - 1
cMRF            print *,'Failure in cut_cross from fix_path: ',
cMRF     .         nmode,k0,rr0
            call stop_run
            return
         endif
...
c: Use .1 km as arbitrary rmin here:
         khi=dcmplx(k_real,db_cut/(8685.9*(.1)))
...
cMRF         print *,'Failure in cut_cross'
         iifail=1
         return
...
co         if(kcub .eq. 0) print *,'kcub = 0 from cut_cross'
...
c
      implicit none
      integer*4 iifail,nctot,ntry,ntry2,iidiag
...
cxx   delki=.05d0*kw
      delki=.01d0*kw
      kneg=k0
5     call r1r2_calc(kneg,rrneg,2,0)
...
55       ntry=ntry+1
         ...
         call r1r2_calc(kneg2,rrneg,2,0)
         if(ntry .gt. 8) then
            kneg=dcmplx(dreal(kneg),dimag(kneg)+.1*delki)
            ntry2=ntry2 + 1
            if(ntry2 .gt. 20) then
cMRF               print *,'Failure to pick up path in move_up_cut2: ',
cMRF     .            k0/kw,kneg/kw
               iifail=1
               return
            endif
...
10    kpos=dcmplx(dreal(kpos),dimag(kpos) + delki)
      ntry=ntry + 1
      if(ntry .gt. 50) then
cMRF         print *,'Failure to find kpos in move_up_cut2: ',k0/kw,kpos/kw
         iifail=1
         return
      endif
      call r1r2_calc(kpos,rrpos,2,0)
...
20    k0=dcmplx(dreal(k0),.5d0*(dimag(kneg)+dimag(kpos)))
      ntry=ntry + 1
      if(ntry .gt. 20) then
cMRF         print *,'Unsuccessful move_up_cut2: ',
cMRF     .         kneg/kw,rrneg(2,4),kpos/kw,rrpos(2,4)
         iifail=1
         return
      endif
      call r1r2_calc(k0,rr0,2,0)
