...
c
      ...
      use i_o_com
      use gen_com
      integer*4 iduct,iidiff,ndup,nc00,nm1
...
cMRF               print *,'traj_sdp unsuccessful '
               return
            endif
         endif
         iidiff=1
...
c: branch:
cMRF                  print *,'Main branches different???  Tell EKW.'
                  iidiff=0
...
c
      ...
      use i_o_com
      use gen_com
      integer*4 ntry1,ntry2,kinc,kdec,ii_usek,j
...
c
      ntry1=ntry1 + 1
      if(ntry1 .gt. 50) then
cMRF         print *,'Failure in traj_sdp: ntry1 = ',ntry1
         iifail=1
         return
      endif
...
20    continue
      ntry2=ntry2 + 1
      if(ntry2 .gt. 4) then
cMRF         print *,'Failure in traj_sdp: ntry2 = ',ntry2,
cMRF     .      maglast,mag_spt,dreal(k_spt)/kw0,dimag(k_spt)*8685.9
cMRF         print *,'Num: ',(ln_spt-lnlast)/(2.*delk),dln_spt,dlnlast
         iifail=1
         return
      endif
      call r1r2_calc(k_spt,r1r2,2,0)
      ln_spt=r1r2(1,4)
      dln_spt=r1r2(2,4)
...
cc   .      dlnlast,dlnr1r2,delk,(lnrrdes-lnlast)/dlnlast
      ...
      k=klast + delk
      ntry=ntry + 1
      call r1r2_calc(k,r1r2,2,0)
      lnr1r2=r1r2(1,4)
      dlnr1r2=r1r2(2,4)
c
      if(ntry .gt. 10) then
cMRF         print *,'ntry>10 in traj_phase: ,ntry,k,k/kw,iish = ',
cMRF     .      ntry,k,k/kw,iish
cMRF         print *,'k,klast,delk = ',k/kw,klast/kw,delk/kw,lnrrdes,
cMRF     .      lnlast,dlnlast
cMRF         print *,'xkbp,iish = ',xkbp(1,1)/kw,iish
cMRF         print *,'num deriv = ',(r1r2(1,4)-lnlast)/(k-klast),dlnlast,
cMRF     .      r1r2(2,4)
         if(ntry .gt. 11) then
            iifail=1
cc          call stop_run
...
c: Follow constant arg(R1R2) to |R1R2|=1 contour:
     ...
     .   rr_st,0.1d0,0.19635d0)
      if(iifail .ne. 0) then
cMRF         print *,'Failure in traj_phase from contour_find2'
         iidiff=0
      endif
c
      return
