      subroutine blug(geo,h,fexp,kt,bp,klay,nadd)
c

      use parms_com
      integer*4 kt,j,klay,nadd,kdo
      real*8 geo(2,5,NLMAX),h(NLMAX),fexp(2,NLMAX),bp(2),
...
c: Create a new layer by splitting current layer:
         call add_layer(kdo,kdo+1,klay,zmax,cmax,geo,h,fexp,
     .      klay+1)
         nadd=nadd + 1
      else
         kdo=kdo + 1
...
ccc
      subroutine add_layer(k,kp1,ktot,zpt,cpt,geo,h,fexp,nlm)
c
      implicit none
      integer*4 k,kp1,ktot,j,j1,j2,ji,nlm
      real*8 zpt,cpt,geo(2,5,nlm),h(nlm),fexp(2,nlm),fac,hold
c
