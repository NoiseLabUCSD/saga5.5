...
c
      jlfake(1)=0
      jlfake(2)=0
c: Default frequency exponent is unity:
      do j=1,NLMAX
         fexp(1,j)=1.d0
         fexp(2,j)=1.d0
      enddo
...
c: Copy geoacoustic parameters of top layers to master array:
         ...
         enddo
c
c: Fill in frequency exponent for bottom layers (fexp):
         if(svp_ver .gt. 2.e0) then
            fexp(1,nlay)=bpt(3,j)
            fexp(2,nlay)=bpt(4,j)
         else
            jb1=1
            if(iabs(ktt(j)) .ne. 1) jb1=3
            if(ktt(j) .lt. 0) then
               fexp(1,nlay)=bpt(jb1,j)
               fexp(2,nlay)=bpt(jb1+1,j)
            endif
         endif
         if(iabs(ktt(j)) .ne. 1) then
            call blug(geo,h,fexp,iabs(ktt(j)),bpt(1,j),nlay,nadd)
            if(iiwrt .eq. 1 .and. nadd .gt. 0) then
               write(2,209)
209            format('   BLUG layer subdivided: ')
               do i=nlay-nadd,nlay
                  write(2,208) h(i),((geo(ji,ii,i),ji=1,2),ii=1,5)
...
c: Copy SVP layers to master arrays:
      jsurf=nlay+1
      do j=1,nsvp-1
         hsvp=zsvp(j+1) - zsvp(j)
         if(hsvp .gt. 0.d0) then
            nlay=nlay+1
            h(nlay)=hsvp
            geo(1,1,nlay)=csvp(j)
            ...
            geo(1,5,nlay)=0.d0
            geo(2,5,nlay)=0.d0
         elseif(iiwrt .eq. 1) then
            print *,'Duplicate depth in SVP ignored: ',j,j+1,zsvp(j)
         endif
      enddo
      jobot=nlay
c
...
cc       if(iabs(ktb(j)) .ne. 1) jb1=3
         if(svp_ver .gt. 2.e0) then
            ...
            jb1=3
            if(ktb(j) .lt. 0) then
               fexp(1,nlay)=bpb(jb1,j)
               fexp(2,nlay)=bpb(jb1+1,j)
            endif
         endif
c
         if(iabs(ktb(j)) .ne. 1) then
            call blug(geo,h,fexp,iabs(ktb(j)),bpb(1,j),nlay,nadd)
            if(iiwrt .eq. 1 .and. nadd .gt. 0) then
               write(2,209)
               do i=nlay-nadd,nlay
                  write(2,208) h(i),((geo(ji,ii,i),ji=1,2),ii=1,5)
...
c
         c_up=geo(2,1,j-1)
         if(c3 .eq. c_up) c_up=geo(1,1,j-1)
         c_dn=geo(1,1,j+1)
         if(c4 .eq. c_dn) c_dn=geo(2,1,j+1)
c
         rhoratx=geo(2,3,j)/geo(1,3,j+1)
         mm_dn=0
cxx      if(rhoratx .gt. 1.5d0 .or. rhoratx .lt. .667d0 .or.
cxx      if(rhoratx .gt. 1.1d0 .or. rhoratx .lt. .909d0 .or.
         if(rhoratx .gt. 1.001d0 .or. rhoratx .lt. .999d0 .or.
     .      j .eq. jflu2) mm_dn=1
c
         if(c3 .eq. c4) then
...
c: FIX 8-4-00:
cc          if(c3 .lt. c_up) then
            if(c3 .lt. c_up .or. mm_up .eq. 1) then
               call duct_enter(j,1,c3,0,nd_next)
            endif
...
c: FIX 8-4-00:
cc          if(c3 .gt. c_up) then
            if(c3 .gt. c_up .or. mm_up .eq. 1) then
...
55       continue
         ...
         if(j .ne. 0) then
            c4=geo(ii,1,j)
            call peak_enter(nduct,j,jlast,c4,c4,1)
...
c: problems in rp_nomm when on negative sheets):
      do j=1,nlay-1
         if(mm(j) .eq. 0 .and. h(j+1) .ne. 0.d0) then
            if(iisol(j) .eq. 1 .and. isp(j) .eq. 1 .and. 
     .         isp(j+1) .eq. 1) then
               print *,'ORCA CANNOT HANDLE ISOSPEED P-WAVE LAYERS '//
     .   'ON BOTH SIDES OF NO-MISMATCH-IN-P S-S INTERFACE!! SORRY!'
               print *,'Layer # ',j,j+1
