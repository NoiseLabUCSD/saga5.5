...
integer*4, dimension(:), allocatable :: nmbb	! NFBBMAX
integer*4 :: nffth1bb,nf1,nf2,iifull,nh_off,lheadw,lrecw
real*8, dimension(:), allocatable :: wbb,kim_bb	! NFBBMAX
real*8 :: df
!real*4, dimension(:), allocatable :: phim_bb	! NFBBMAX
real*4 :: xhbb(20)
complex*8, dimension(:,:), allocatable :: phibb,dpsibb	! NZ_NF_MAX
!cc
!cc
!     common /geo_com/ 
...
real*8 :: rholay(2),f_hz,w,wsq,kw,kw0,cref,phtot,rhorat(NLMAX),&
	...
	rho_duct,phcut,chspmax,khspmin,f_min,f_max,&
	geo_fake(2,5,NDMAX)
...
real*4 :: theta(NSRMAX),xh(20),xkhr(NSRMAX),xkhi(NSRMAX)
...
integer*4 :: nzmx(NLMAX),nzmxtot,jzmx(NLMAX),jsrmx(NSRMAX),&
	mx_m(NSRMAX),nctot,nclast,nrleg(NSEGMAX),&
	ncall,ncmat,nm_max2,jsr2j(NSRMAX),aisoln(2,NLMAX),nsave,&
	nmerge,nskip,nmode_hdf
integer*4, dimension(:), allocatable :: ncalc,jmlink	! NM_MAX
real*4, dimension(:,:), allocatable :: r4mat1,r4mat2,r4mat3,r4mat4,r4mat5,r4mat6,pmat
complex*8, dimension(:), allocatable :: phi_f
real*4, dimension(:,:,:), allocatable :: r4mat_3d
real*4, dimension(:), allocatable :: var_ax,xmode
real*4 :: t_src(NRNGMAX),rec_lab(NSRMAX),zsrmin,zsrmax,NaN
!cc
!cc
...
complex*8 :: phisr(NSRMAX),dpsisr(NSRMAX)
...
integer*4 :: iia1b1(NLMAX),indx_duct(NDMAX),ndrx,jl2jd(NLMAX),&
	jpeak(NDMAX),jval(2,NDMAX),nlwz,jlwz(NLMAX),&
	nzoff,jlayx(NDMAX)
real*8 :: dk_max,dk_max0,g11(6,2,NLMAX),g12(6,2,NLMAX),&
	g21(6,2,NLMAX),g22(6,2,NLMAX),g_exp(6,NLMAX),&
	h11(6,2,NLMAX),h12(6,2,NLMAX),h21(6,2,NLMAX),&
	h22(6,2,NLMAX),h_exp(6,NLMAX),u11(6,NLMAX),u12(6,NLMAX),&
	u21(6,NLMAX),u22(6,NLMAX),u_exp(6,NLMAX),migam2(6),&
	rhofac(NLMAX),rho_prod(2,NLMAX),&
	Dphix_w(NSRMAX),Ddphix_w(NSRMAX),Dphi_w(2,NLMAX),&
	Ddphi_w(2,NLMAX),&
	am(NLMAX),bm(NLMAX),betm(NLMAX),gm(NLMAX),rhom(NLMAX),&
	xilay(2,NLMAX),cspan(0:NDMAX),cduct(NDMAX),dz_duct(NDMAX),&
	zpeak(NDMAX+1),ccr_lo,ccr_hi
real*8, dimension(:,:), allocatable :: Dphiz_w,Ddphiz_w,ekn
!cc
!cc
