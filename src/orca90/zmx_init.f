...
c
      ...
      real*8 z1,z2,rhfac,fac,c1invsq,c2invsq,cfac
...
c         do while(zsr(jj) .lt. z1 .and. jj .lt. nzsr)
c            jj=jj+1
c         enddo
c         jj1=jj
c         do while(zsr(jj) .lt. z2 .and. jj .lt. nzsr)
c            jj=jj+1
c         enddo
c         jj2=jj-1
c         if(jj .eq. nzsr) jj2=jj

         jj1=nzsr+1
         do jj=jj2+1,nzsr
            if(zsr(jj) .gt. z1 .and. zsr(jj) .le. z2) then
               jj1=min(jj1,jj)
               jj2=jj
            endif
            if(zsr(jj) .gt. z2) goto 12
         enddo
12       continue

         jzmx(j)=jptr+1
         nzmx(j)=max(0,jj2-jj1+1)
...
cc       c1inv=dcmplx(1.d0,geo(2,4,j)*geo(2,1,j)/fac)/geo(2,1,j)
         c1invsq=1.d0/(geo(2,1,j)*geo(2,1,j))
cc       c2inv=dcmplx(1.d0,geo(1,4,j)*geo(1,1,j)/fac)/geo(1,1,j)
         c2invsq=1.d0/(geo(1,1,j)*geo(1,1,j))
...
c: jsrmx(jptr)=jsr, the index of depth zmx(jptr) in the array zsr(1:nzsr):
            ...
            cp_sr(jsr)=1.d0/dsqrt(c1invsq + zmx(jptr)*cfac)
            cs_sr(jsr)=(0.d0,0.d0)
...
c: Pointer from zsr to geo [find s/r depth zsr(jsr) in geo(:,:,j)]:
         ...
         if(iisol(j) .eq. 1 .and. jj2 .ge. jj1) then
            c1invsq=1.d0/(geo(2,2,j)*geo(2,2,j))
            c2invsq=1.d0/(geo(1,2,j)*geo(1,2,j))
            cfac=(c2invsq - c1invsq)/dmax1(1.d-100,h(j))
            do jsr=jj2,jj1,-1
               kksh(jsr)=1
               cs_sr(jsr)=1.d0/dsqrt(c1invsq + zmx(jptr)*cfac)
            enddo
         endif
...
c        if(jj2 .eq. nzsr) jj2=jj
         ...
         do jj=jj2+1,nzsr
            ...
            if(zsr(jj) .gt. z2) goto 14
         enddo
14       continue
c
         jzmx(j)=jptr+1
         nzmx(j)=max(0,jj2-jj1+1)
         c1invsq=1.d0/(geo(1,1,j)*geo(1,1,j))
         c2invsq=1.d0/(geo(2,1,j)*geo(2,1,j))
         cfac=(c2invsq - c1invsq)/dmax1(1.d-100,h(j))
...
c: Pointer from zsr to geo [find s/r depth zsr(jsr) in geo(:,:,j)]:
         ...
         if(iisol(j) .eq. 1 .and. jj1 .le. jj2) then
            c1invsq=1.d0/(geo(1,2,j)*geo(1,2,j))
            c2invsq=1.d0/(geo(2,2,j)*geo(2,2,j))
            cfac=(c2invsq - c1invsq)/dmax1(1.d-100,h(j))
            do jsr=jj1,jj2
               kksh(jsr)=1
               cs_sr(jsr)=1.d0/dsqrt(c1invsq + zmx(jptr)*cfac)
