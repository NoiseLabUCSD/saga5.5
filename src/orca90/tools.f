...
c: Call with +1 instead of -1 since Alliant FFT has +i in inverse FFT:
      call four1(x,n,-1)
c	 four1 requires a real*4 as first argument
...
ccc
      subroutine hpsort_r4_indx(n,ra,indx)
c
      implicit none
      integer*4 n,indx(n)
      real*4 ra(n),rra
      integer*4 i,ir,j,l
c: Sorts a real*4 array ra(1:n) into ascending order using the Heapsort
c: algorithm. n is input; ra is replaced on output by its sorted
c: rearrangement.
c
      integer*4 iia
...
c
      integer nfile,lenrec,lfft,nfft
...
c: FFT files are defined to be big endian; 
c: Leave off convert='big_endian' if compliler can't handle it
     .   access='direct',recl=lenrec,status='replace')
c: FFT files are defined to be big endian; 
c: Leave off convert='big_endian' if compliler can't handle it
