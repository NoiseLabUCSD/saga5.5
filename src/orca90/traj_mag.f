...
c
     ...
     .   mag_err,eph_rat,mag_rat,krat,dph_left,mag_fac,delph,magsq,
     .   magsq_knd
c
      npt=0
      mag_fac=1.d0
...
c: Reset ph_step so that it does not start up again at very small values:
            ph_step=twpie/nint(phfac0)
            iifail=2
            if(iidiag .ge. 1) print *,'traj_mag stuck at XKREF: ',
     .         dreal(k)/kw0,dimag(k)*8685.9
            return
         endif
cMRF         print *,'ntry>10: ,ntry,k,k/kw0,iish = ',ntry,k,k/kw0
cMRF         print *,'k,klast,delk = ',k/kw0,klast/kw0,delk/kw0
cMRF         print *,'lnrr,lnlast,lnrrdes = ',lnr1r2,lnlast,lnrrdes
cMRF         print *,'xkbp,iish = ',xkbp(1,1)/kw0,iish
cMRF         print *,'xkref,iish_ref = ',xkref/kw0,iish_ref
cMRF         print *,'num deriv = ',(lnr1r2-lnlast)/(k-klast),dlnlast,
cMRF     .      dlnr1r2
...
cc    if(dreal(k) .lt. kremin .and. real(delk) .lt. 0.d0) then
     ...
     .         'terminating due to small Re(k) (see cphmax).'
...
co            print *,'ntryx>100'
         ...
         if(ntryx .gt. 105) then
cMRF            print *,'Total # tries exceeded 105 in traj_mag: ',
cMRF     .         k/kw0,iish,dph_left
            iifail=1
cc          call stop_run
            return
         endif
...
c
     ...
     .      lnlast,dlnlast
         ...
         print *,'num deriv = ',(r1r2(1,4)-lnlast)/(k-klast),dlnlast,
     .      r1r2(2,4)
         if(ntry .gt. 11) then
            iifail=1
cc          call stop_run
