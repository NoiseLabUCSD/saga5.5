...
c---- READ the optimization PARAMETER
      ...
      DO i=1,nparm
         IF (fmin(i).GT.fmax(i))THEN
            ...
            ierr=1
         ENDIF
         IF (par2phy(i).LT.1 .OR. par2phy(i).GT.28 .OR.
     &        par2phy(i).EQ.10 )THEN
            WRITE(*,*)' *** par2phy not correct, parm',i, par2phy(i)
            ierr=1
         ENDIF
...
c      write(*,*) 'afteroutplane: rng = ',(rng(ii),ii=1,ndep)
      ...
      DO ifreqy=1,nfrq
         ...
         MINMOD=1
         MAXMOD=maxnomode
c         WRITE(*,*)' min and max range from snaprdinit',secd
c       write(*,*)'sector length', R_slngth(1)
