c***************************
      ...
      INCLUDE 'comforw.h'
      REAL fit,fit2
      REAL*8 fitlong,e1,e2,e3,fitlag
      INTEGER i,j,ix,ifrq,ibart,j1start,jdep,index,index2
      INTEGER index3
      COMPLEX*16 cc,cx,cc_diag
...
c                WRITE(*,*)'cx',cx,cov(index2),resp(index),index2,index
               ...
               ENDIF
               fit_bart_ind(ibart)=xcov_trace(ifrq)-dreal(cc)
...
c     ---        each freq is weigthed according to received power
                  fit_bart_ind(jdep)=e2-ABS(cc)**2/(e1)
               ELSEIF ((isubopt(5).EQ.2).or. (isubopt(5).EQ.4)) THEN !f1
c     ----    each frequency is weighted the same.
                  fit_bart_ind(jdep)=1-ABS(cc)**2/(e1*e2)
               endif 
               IF ((isubopt(5).EQ.1) .or. (isubopt(5).EQ.2)) THEN              
                  fit_long =fit_long+fit_bart_ind(jdep)
               ELSEIF ((isubopt(5).EQ.3) .or. (isubopt(5).EQ.4)) THEN
                  fit_long =fit_long*fit_bart_ind(jdep)**(1.0/ndep/nx)
               ENDIF
             ENDDO               ! nx-loop
         ENDDO                  ! ndep--loop
         IF ((isubopt(5).EQ.1) .or. (isubopt(5).EQ.2)) THEN              
            fit=1-fit_long/ndep/nx   ! nfrq
         ELSEIF ((isubopt(5).EQ.3) .or. (isubopt(5).EQ.4)) THEN
            fit=real(fit_long)+1e-11
         ENDIF
c               write(*,*)'fit_long', fit_long
c               write(*,*)'fit_long', fit_long
         GOTO 900
      ENDIF                     ! iopt(5)=5
         ...
         DO jdep=1,ndep 
            DO ifrq=1,nfrq
               ...
               DO ilag=-10,10
                  ...
                  iup=MIN(nx,nx-ilag)
                  DO i=ilow,iup
                     fitlag=fitlag+(REAL(DATA(i+index))*e2
     1                    -REAL(resp(i+index+ilag))*e1)**2
c     resp(i+index)=amp*ABS((resp(i+index)))
                     
c     WRITE(*,*)' cost' ,(i+index),resp(i+index)
                  ENDDO
                  fitlag=fitlag*nx/(nx-ABS(ilag))
c     WRITE(*,*)ilag,ilow,iup,fitlag
                  IF (fitlag.LT.fitlong) THEN
                     fitlong=fitlag
                     iminlag=ilag
c     WRITE(*,*)' fitlong,iminlag', fitlong,iminlag
                  ENDIF
               ENDDO
c     WRITE(*,*)'cost tpem', resp(nx+index),nx,amp
